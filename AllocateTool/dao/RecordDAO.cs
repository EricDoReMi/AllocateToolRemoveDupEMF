...
namespace AllocateTool.dao
{
    public partial class RecordDAO : BaseDAO<Record>
    {
        ...
        public List<Record> FindAllRecords(OleDbConnection conn)
        {
            string sqlStr = @"SELECT * FROM records";
            OleDbParameter[] paras = { };
            List<Record> list = queryEntity(conn, sqlStr, paras);
            return list;
        }

        public void DeleteRecordsByID(OleDbConnection conn,long id)
        {
            string sqlStr = @"Delete FROM records Where m_id=@M_id";
